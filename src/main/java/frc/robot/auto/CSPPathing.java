...
public final class CSPPathing {
  ...
  // Per-match state — reset in reset()
  ...
  /**
   * Reset per-match path state.
   *
   * <p>Call this from autonomousInit() to clear lastPose between matches so stale state from match
   * N does not affect match N+1 (both matches run in the same roboRIO JVM session).
   */
  ...
  // ── Public generatePath overloads ──────────────────────────────────────────

  /**
   * Generate a path from {@code start} to {@code end}.
   *
   * <p>FIX: All overloads previously replaced the caller-supplied {@code start} with the internal
   * {@code lastPose} after the first call, silently discarding the caller's start pose for every
   * subsequent invocation. This caused all subsequent auto paths to begin from the wrong field
   * position.
   *
   * <p>The fix: {@code start} is always honoured. {@code lastPose} is still updated to {@code end}
   * so that {@link #robotOnPath(Pose2d)} continues to work correctly. Call {@link #reset()} between
   * matches (already done in {@code autonomousInit()}).
   */
  ...
  /**
   * Generate a path from {@code start} to a {@code Translation2d} endpoint (heading inferred from
   * travel direction).
   */
  public static List<Waypoint> generatePath(Pose2d start, Translation2d end) {
    Pose2d endPose = new Pose2d(end, new Rotation2d());
    lastPose = endPose;
    return generatePathInternal(start, new Pose2d[]{endPose});
  }

  /** Generate a path through one or more intermediate poses to a final pose. */
  ...
  /** Generate a path through one or more intermediate translations to a final translation. */
  public static List<Waypoint> generatePath(Pose2d start, Translation2d... midAndEnd) {
    if (midAndEnd == null || midAndEnd.length == 0)
      throw new IllegalArgumentException("Must provide at least end Translation2d");
    Pose2d[] poses = new Pose2d[midAndEnd.length];
    for (int i = 0; i < midAndEnd.length; i++) {
      poses[i] = new Pose2d(midAndEnd[i], new Rotation2d());
    }
  ...
  private static List<Waypoint> generatePathInternal(Pose2d start, Pose2d[] midAndEnd) {
    ...
    if (globalConfig == null)
      throw new IllegalStateException(
          "Call configureConstraints(...) with a RobotConfig before generatePath");
    ...
    // 2) Initial headings (point toward next)
    ...
    for (int pass = 0; pass < maxPasses; pass++) {
      ...
      double refineRange = (pass == 0) ? ANGLE_RANGE_DEG : Math.min(ANGLE_RANGE_DEG, COARSE_STEP_DEG);
        ...
        for (double off = -refineRange; off <= refineRange; off += step) {
          ...
          if (ev == null) {
            if (controlsIntersectAnyObstacle(anchors, headings)) {
              ...
              try {
                traj = new PathPlannerTrajectory(path, new ChassisSpeeds(), new Rotation2d(), globalConfig);
              } catch (Throwable ex) {
                ev = new Evaluation(true, Double.POSITIVE_INFINITY, Double.POSITIVE_INFINITY);
                cache.put(key, ev);
                continue;
  ...
  private static boolean segmentIntersectsAnyInflatedObstacle(Translation2d a, Translation2d b) {
    for (RectObstacle r : inflated)
      if (r.intersectsSegmentAnalytic(a, b)) return true;
    return false;
  }
  ...
  private static Evaluation evaluateTrajectory(PathPlannerTrajectory traj) {
    ...
    for (double t = 0.0; t <= totalT + 1e-9; t += SAMPLE_DT) {
      ...
      if (prev != null) {
        ...
        len += d;
        double dtheta = normalizeAngle(s.heading.getRadians() - prev.heading.getRadians());
        if (d > CURV_EPS_DIST) {
  ...
  public static GoalEndState guessGoalEndStateFromWaypoints(List<Waypoint> waypoints) {
    ...
    if (waypoints.size() >= 2) {
      Translation2d prevAnchor = waypoints.get(waypoints.size() - 2).anchor();
      if (prevAnchor != null) return new GoalEndState(0.0, headingFrom(prevAnchor, anchor));
    }
    return new GoalEndState(0.0, new Rotation2d());
  }
  ...
  private static List<Translation2d> runAStarWithLinks(Translation2d start, Translation2d goal) {
    ...
    for (NavNode nn : navNodes) {
      ...
      for (String linkId : nn.links) {
        ...
        if (b != null) a.neighbors.add(b);
      }
    }

    List<GraphNode> nodeList = new ArrayList<>(graph.values());

    GraphNode nearestToStart = null, nearestToGoal = null;
    double bestStartDist = Double.POSITIVE_INFINITY, bestGoalDist = Double.POSITIVE_INFINITY;
    for (GraphNode node : nodeList) {
      Translation2d t = node.nav.pose.getTranslation();
      double ds = t.getDistance(start);
      if (ds < bestStartDist) { bestStartDist = ds; nearestToStart = node; }
      double dg = t.getDistance(goal);
      if (dg < bestGoalDist) { bestGoalDist = dg; nearestToGoal = node; }
    }

    try {
      DataLogManager.log("[CSPPathing] A* nearestStart: "
          + (nearestToStart != null ? nearestToStart.nav.id : "NULL")
          + " nearestGoal: "
          + (nearestToGoal != null ? nearestToGoal.nav.id : "NULL"));
    } catch (Throwable ignored) {}

    java.util.function.BiFunction<GraphNode, GraphNode, List<GraphNode>> runAstar =
        (sNode, gNode) -> {
          ...
          while (!open.isEmpty()) {
            ...
            if (cur == gNode) {
              ...
              GraphNode p = cur;
              while (p != null) { path.add(p); p = p.parent; }
            ...
            for (GraphNode nb : cur.neighbors) {
              ...
              if (tentative < nb.g) {
                nb.parent = cur;
                nb.g = tentative;
                nb.f = tentative + distance(nb.nav.pose.getTranslation(), gNode.nav.pose.getTranslation());
    ...
    List<GraphNode> path = runAstar.apply(nearestToStart, nearestToGoal);

    if (path == null) {
      double bestGoalDistAfter = Double.POSITIVE_INFINITY;
      List<GraphNode> bestCandidate = null;
      for (GraphNode candGoal : nodeList) {
        List<GraphNode> p = runAstar.apply(nearestToStart, candGoal);
        if (p == null) continue;
        double lastDist = p.get(p.size() - 1).nav.pose.getTranslation().getDistance(goal);
        if (lastDist < bestGoalDistAfter) { bestGoalDistAfter = lastDist; bestCandidate = p; }
      }
      path = bestCandidate;
    }
    ...
    if (path == null) {
      ...
      for (GraphNode sNode : nodeList) {
        for (GraphNode gNode : nodeList) {
          List<GraphNode> p = runAstar.apply(sNode, gNode);
          if (p == null || p.isEmpty()) continue;
          double pathLen = 0.0;
          for (int i = 0; i < p.size() - 1; i++)
            pathLen += p.get(i).nav.pose.getTranslation().getDistance(p.get(i + 1).nav.pose.getTranslation());
          double total = start.getDistance(p.get(0).nav.pose.getTranslation())
              + pathLen
              + p.get(p.size() - 1).nav.pose.getTranslation().getDistance(goal);
          if (total < bestTotal) { bestTotal = total; bestPath = p; }
        }
      }
      path = bestPath;
    }
    ...
    if (path == null) {
      DataLogManager.log("[CSPPathing] No path found — falling back to straight line.");
    ...
    DataLogManager.log("[CSPPathing] A* node path: " + sb);
    return result;
  }
  ...
  /**
   * Check whether the robot is within {@value #PATH_ERROR_METERS} m of the last path endpoint.
   *
   * @param robotPose Current robot pose from odometry.
   * @return true if on-path or within tolerance; false if off-path or no path generated yet.
   */
  ...
  private static final class GraphNode {
    ...
    GraphNode(NavNode nav) { this.nav = nav; }
  }
